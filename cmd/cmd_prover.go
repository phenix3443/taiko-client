...
var (
	...
	ProveBlockTxReplacementMultiplierFlag = &cli.Uint64Flag{
		...
		Destination: &proverConf.ProveBlockTxReplacementMultiplier,
		Category:    proverCategory,
		Action: func(c *cli.Context, v uint64) error {
			proverConf.ProveBlockTxReplacementMultiplier = v
			return nil
		},
	}
	ProveBlockMaxTxGasTipCapFlag = &cli.Uint64Flag{
		...
		Category: proverCategory,
		Action: func(c *cli.Context, v uint64) error {
			proverConf.ProveBlockMaxTxGasTipCap = new(big.Int).SetUint64(v)
			return nil
		},
	}
	TempCapacityExpiresAtFlag = &cli.DurationFlag{
		Name:        "prover.tempCapacityExpiresAt",
		Usage:       "time in seconds temporary capacity lives for, format: 36s",
		Value:       36 * time.Second,
		Category:    proverCategory,
		Destination: &proverConf.TempCapacityExpiresAt,
		Action: func(c *cli.Context, v time.Duration) error {
			proverConf.TempCapacityExpiresAt = v
			return nil
		},
	}
)
...
var proverFlags = MergeFlags(CommonFlags, []cli.Flag{
	...
	ProveBlockTxReplacementMultiplierFlag,
	ProveBlockMaxTxGasTipCapFlag,
	GraffitiFlag,
